$HzVersion="3.10-BETA-1"
$HzTestVersion="3.10-SNAPSHOT"
$HazelcastTestVersion=$HzTestVersion
...
If ($HzTestVersion -like "*-SNAPSHOT") {
    $TestRepo=$SnapshotRepo
    $EnterpriseTestRepo=$EnterpriseSnapshotRepo
} Else {
    $TestRepo=$ReleaseRepo
    $EnterpriseTestRepo=$EnterpriseReleaseRepo
}
...
If (Test-Path -PathType Leaf "hazelcast-$HazelcastTestVersion-tests.jar") {
	...
	mvn -q dependency:get "-DrepoUrl=$TestRepo" "-Dartifact=com.hazelcast:hazelcast:$HazelcastTestVersion`:jar:tests" "-Ddest=hazelcast-$HazelcastTestVersion-tests.jar"
	If ($lastExitCode -ne 0) {
